...
const main = async () => {
  ...
  const createMachine = async (name, accessToken, attempt, promise) => {
    let loading;
    promise = promise || new Promise((resolve, reject) => {});

    if (attempt === 6) {
      console.log('Creating droplet failed on 5 attempts, giving up.')
      promise.reject();
    }

    loading = ora(`Creating droplet (try ${attempt} of 5)...`).start();
    await createDockerMachine(name, accessToken).then(result => {
      loading.succeed();
      promise.resolve();
    }).catch(async (err) => {
      loading.fail();
      if (err.code === 1) {
        console.log('There was an error while creating the droplet...');
        console.log('The machine either already exists or Docker Machine encountered a race condition.');
        console.log('We\'ll remove the node and reattempt creating...');
      } else if (err.code === 3) {
        console.log('There was an error while creating the droplet...');
        console.log('The error appears to be related to authentication.')
        console.log('We\'ll remove the node and reattempt creating...');
      }

      loading = ora('Removing droplet...').start();
      execSync('docker-machine rm -y mothership-paas');
      loading.succeed();
      createMachine(name, accessToken, attempt + 1, promise);
    });
  };

  while (true) {
    // KNOWN ERROR CODES
    // 1 — Docker Machine already exists OR sudo apt-get update error
    // 3 - Unable to authenticate
    await createMachine(MOTHERSHIP_NODE_NAME, accessToken, 5);
  }
